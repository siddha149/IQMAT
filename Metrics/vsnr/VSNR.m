...
%    VIEWING_PARAMS.g = display device gamma 
...
%  mex shared library from 'imdwt_cpp/imdwt.cpp'.)

%  Author: D. M. Chander
%  Copyright 1998-2008 Image Coding and Analysis Lab, Oklahoma State 
%  University, Stillwater, OK 74078 USA.
%  $Revision: STB 1.1.0.0 $  $Date: 2008/06/02/ 13:00:00 $ 
...
if (src_img ~= -1)
  ...
  if isa(src_img, 'double')
    ...
    vsnr_data.src_img = double(src_img);
  end
  clear src_img;
  analyze_src_img;
...
if (vsnr_data.Ce > vsnr_data.CTe)
  ...
  res(find(vsnr_data.Ces < eps)) = inf;
else
